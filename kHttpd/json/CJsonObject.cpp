...
double CJsonObject::toDouble() const {
    if (m_pJsonData != NULL && (m_pJsonData->type == cJSON_Double || m_pJsonData->type == cJSON_Int)) {
        return m_pJsonData->valuedouble;
               ...
               (m_pExternJsonDataRef->type == cJSON_Double || m_pExternJsonDataRef->type == cJSON_Int)) {
...
bool CJsonObject::Get(const std::string &strKey, long &uiValue) const {
    cJSON *pJsonStruct = NULL;
    if (m_pJsonData != NULL) {
        if (m_pJsonData->type == cJSON_Object) {
            pJsonStruct = cJSON_GetObjectItem(m_pJsonData, strKey.c_str());
    ...
    if (pJsonStruct->type == cJSON_Int) {
        uiValue = (long)(pJsonStruct->valueint);
        return (true);
    } else if (pJsonStruct->type == cJSON_Double) {
        uiValue = (long)(pJsonStruct->valuedouble);
        return (true);
    }
    return (false);
}
...
bool CJsonObject::Get(const std::string &strKey, cJSON_int64 &llValue) const {
    cJSON *pJsonStruct = NULL;
    if (m_pJsonData != NULL) {
        if (m_pJsonData->type == cJSON_Object) {
            pJsonStruct = cJSON_GetObjectItem(m_pJsonData, strKey.c_str());
    ...
    if (pJsonStruct->type == cJSON_Int) {
        llValue = (cJSON_int64) (pJsonStruct->valueint);
        return (true);
    } else if (pJsonStruct->type == cJSON_Double) {
        llValue = (cJSON_int64) (pJsonStruct->valuedouble);
        return (true);
    }
    return (false);
}
...
bool CJsonObject::Add(const std::string &strKey, cJSON_int64 llValue) {
    cJSON *pFocusData = NULL;
    if (m_pJsonData != NULL) {
        pFocusData = m_pJsonData;
    } else if (m_pExternJsonDataRef != NULL) {
...
bool CJsonObject::Replace(const std::string &strKey, cJSON_int64 llValue) {
    cJSON *pFocusData = NULL;
    if (m_pJsonData == NULL) {
        pFocusData = m_pExternJsonDataRef;
    } else {
...
bool CJsonObject::Get(int iWhich, cJSON_int64 &llValue) const {
    cJSON *pJsonStruct = NULL;
    if (m_pJsonData != NULL) {
        if (m_pJsonData->type == cJSON_Array) {
            pJsonStruct = cJSON_GetArrayItem(m_pJsonData, iWhich);
...
bool CJsonObject::Add(cJSON_int64 llValue) {
    cJSON *pFocusData = NULL;
    if (m_pJsonData != NULL) {
        pFocusData = m_pJsonData;
    } else if (m_pExternJsonDataRef != NULL) {
...
bool CJsonObject::AddAsFirst(cJSON_int64 llValue) {
    cJSON *pFocusData = NULL;
    if (m_pJsonData != NULL) {
        pFocusData = m_pJsonData;
    } else if (m_pExternJsonDataRef != NULL) {
...
bool CJsonObject::Replace(int iWhich, cJSON_int64 llValue) {
    cJSON *pFocusData = NULL;
    if (m_pJsonData == NULL) {
        pFocusData = m_pExternJsonDataRef;
    } else {
